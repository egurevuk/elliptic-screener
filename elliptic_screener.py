"""
Elliptic Wallet Screener — with Google Auth + Usage Stats
...
# ── Google Auth ───────────────────────────────────────────────────────────────
def login_wall():
    """Block access until user signs in with Google. Returns user object."""
    sb = get_supabase()

    if "user" not in st.session_state:
        st.session_state.user = None

    # Handle OAuth callback — Supabase appends ?code= to the redirect URL
...
def _render_login_page():
    ...
    with col2:
        ...
        st.markdown("Screen USDT (TRC-20) wallets for AML exposure and sanctions risk.")
        st.markdown(" ")
...
def render_admin_stats():
    ...
    try:
        ...
        flagged = df["verdict"].isin(["🟠 Medium Risk", "🔴 High Risk"]).sum()
        m4.metric("Flagged",        int(flagged))
...
def render_not_found(address):
    ...
    exchanges = [
        {"name": "Binance",  "url": "https://www.binance.com/en/register",  "desc": "World's largest exchange. Fast TRC-20 withdrawals, low fees.",              "icon": "🟡"},
        {"name": "Bybit",    "url": "https://www.bybit.com/en/register",    "desc": "Simple onboarding, instant USDT withdrawals.",                              "icon": "🟠"},
        {"name": "OKX",      "url": "https://www.okx.com/join",             "desc": "Broad USDT support across all major networks including TRC-20.",             "icon": "⚫"},
        {"name": "Kraken",   "url": "https://www.kraken.com/sign-up",       "desc": "US-regulated, strong security and fiat on-ramps.",                          "icon": "🔵"},
        {"name": "KuCoin",   "url": "https://www.kucoin.com/register",      "desc": "Easy TRC-20 USDT withdrawals and low minimums.",                            "icon": "🟢"},
    ]
...
def render_cluster_entities(data):
    ...
    if isinstance(data.get("cluster_entities"), list):
        ...
        raw = data["analysed_by"].get("cluster_entities", [])
...
def render_eval_rules(rules, direction_label):
    ...
    for rule in rules:
        ...
        with st.expander(f"{'🔴' if risk_10>=5 else '🟠' if risk_10>=1 else '🟢'} **{rule_name}** — Score: {risk_10:.4f}/10"):
            for elem in (rule.get("matched_elements") or []):
                ...
                if contrib_dicts:
                    ...
                    if sanctioned:
                        st.error("⛔ **Sanctioned entities:** " + " | ".join(r["Entity"] for r in sanctioned))
                st.divider()
...
def render_contribution_side(items, label):
    ...
    chart = [{"Entity": (c.get("entities") or [{}])[0].get("name","?") if isinstance((c.get("entities") or [{}])[0], dict) else "?",
              "USD": float((c.get("contribution_value") or {}).get("usd") or 0)}
             for c in items if (c.get("contribution_value") or {}).get("usd")]
    chart = [r for r in chart if r["USD"] > 0]
    if chart:
...
def render_metadata(data, address):
    ...
    rows = [("Screening ID", data.get("screening_id")), ("Report ID", data.get("id")),
            ("Type", data.get("type")), ("Asset Tier", data.get("asset_tier")),
            ("Process Status", data.get("process_status")), ("Workflow Status", data.get("workflow_status")),
            ("Screening Source", data.get("screening_source")), ("Team ID", data.get("team_id")),
            ("Created At", fmt_ts(data.get("created_at"))), ("Analysed At", fmt_ts(data.get("analysed_at")))]
...
def main():
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        st.markdown("**Upload an Excel (.xlsx) or CSV file.** The tool looks for a column named `address`, `wallet`, or `hash`. One wallet per row.")
        ...
        if uploaded:
            try:
                ...
                st.error(f"Missing dependency: `{missing}`. Add it to `requirements.txt`.")
                st.stop()
            except Exception as e:
            ...
            with col_delay:
                delay = st.number_input("Delay between requests (s)",
                ...
                for i, addr in enumerate(addresses):
                    ...
                    if not addr.startswith("T"):
                        ...
                        results_rows.append({addr_col: addr, **extra_vals,
                            "Risk Score":"—","Verdict": verdict,
                            "Source Risk":"—","Dest Risk":"—",
