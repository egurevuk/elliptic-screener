"""
Elliptic Wallet Screener — Full Detail Report + Google Auth
...
# ── Google Auth ───────────────────────────────────────────────────────────────
def login_wall():
    """Show login page until user authenticates. Returns user email string."""
    sb = get_supabase()

    if "user" not in st.session_state:
        st.session_state.user = None

    # Handle ?code= callback that Supabase appends after Google login
